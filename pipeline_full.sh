...
train(){
    ...
    #! TRAINING ARGS
    EPOCHS=3 #! full training 3 epochs
    SEED=42 #! default 42
    LR=2e-5 #! 2e-5 for full training, 3e-4 or 2e-4 or 1e-4 for lora
    ...
    #!OUTPUT
    OUTPUT_MODEL_DIR=${OUTPUT_BASE_DIR}/full/${MODEL_NAME}/${DATASET_TYPE}/${WANDB_RUN_NAME}
    ...
    if [ "${5}" = 'train' ]; then
        deepspeed --include=localhost:${DEVICE} src/official_train_v3.py \
...
inference(){
    ...
    BASE_DIR=/apdcephfs/share_916081/effidit_shared_data/chillzhang
    #! OUTPUT_MODEL_DIR
    PRETRAINED_MODEL_PATH=${OUTPUT_MODEL_DIR}
    # LORA_WEIGHTS_PATH=${OUTPUT_MODEL_DIR}
    PRETRAINED_TOKENIZER_PATH=${OUTPUT_MODEL_DIR}
    # #! change test file
...
ddp_inference(){
    ...
    # "$str1" == "$prefix"* 
    # "${DATA_TYPE}" = "X_CSQA"
    if [[ "${DATA_TYPE}" == *"X_CSQA"* ]]; then
        LANG=$(basename $(dirname ${TEST_FILE_PATH}))
        if [ "${4}" = "True" ]; then
            OUTPUT_FILE_DIR=${OUTPUT_MODEL_DIR}/predictions/${DATA_TYPE}/${LANG}/${SRC_LANG}-${TGT_LANG}/${PROMPT_TYPE}
        else
            OUTPUT_FILE_DIR=${OUTPUT_MODEL_DIR}/predictions/${DATA_TYPE}/${LANG}/no-add-lang/${PROMPT_TYPE}
        fi
        
    else 
        if [ "${4}" = "True" ]; then
            OUTPUT_FILE_DIR=${OUTPUT_MODEL_DIR}/predictions/${DATA_TYPE}/${SRC_LANG}-${TGT_LANG}/${PROMPT_TYPE}
        else
            OUTPUT_FILE_DIR=${OUTPUT_MODEL_DIR}/predictions/${DATA_TYPE}/no-add-lang/${PROMPT_TYPE}
        fi
    fi
...
# DATASETS=('alpaca_data+zh_alpaca_data_revised.json')
...
DATASET_TYPES=('alpaca_data')
# declare -a DATASIZES
# DATA_SIZES=('50' '100' '200' '300' '400' '500' '600' '700' '800' '900' '1000' '5000' '10000')
# DATA_SIZES=('500' '600' '700' '800' '900' '1000' '5000' '10000')
# DATA_SIZES=('500' '1000' '2000' '3000' '4000' '5000' '6000' '7000' '8000' '9000' '10000')
...
train_and_inference_different_dataset(){
    ...
    do 
        ...
        do 
            train "${PRETRAINED_MODELS[${i}]}" "${MODEL_NAMES[${i}]}" "${DATASET}"
            for ADD_LANG in True False
            do 
                inference "${MODEL_NAMES[${i}]}+${DATASET}" "${ADD_LANG}"
            done
        done
    done
}
...
train_and_inference_different_size(){
    ...
    do 
        ...
        do  
            for DATASET in "alpaca_mix_${DATA_SIZE}_zh_lang_type_0_data.json"
            do
                train "${PRETRAINED_MODELS[${i}]}" "${MODEL_NAMES[${i}]}" "${DATASET}" train
                for ADD_LANG in True
                do 
                    inference "${MODEL_NAMES[${i}]}+${DATASET}" "${ADD_LANG}"
                done
            done
...
eval_x_csqa(){
    ...
    # DATA_TYPE=${1} #! X_CSQA, X_CSQA_ARC_Easy, X_CSQA_ARC_Challenge, X_CSQA_PIQA
    # declare -a TEST_LANGS=('en')
    declare -a EVAL_DATA_TYPES=('X_CSQA_ARC_Easy' 'X_CSQA_ARC_Challenge' 'X_CSQA_PIQA') #'X_CSQA'
    ...
    do
        ...
        do 
            for j in ${!DATASETS[@]}
            do
                train "${PRETRAINED_MODELS[${i}]}" "${MODEL_NAMES[${i}]}" "${DATASETS[${j}]}" "${EVAL_DATASET_TYPES[${j}]}" eval
                for ADD_LANG in False
                do 
                    for LANG in ${TEST_LANGS[@]}
                    do
                    TEST_FILE=datas/X-CSR_datasets/${EVAL_DATA_TYPE}/${LANG}/dev_v2.jsonl
                    ddp_inference "${MODEL_NAMES[${i}]}+${DATASETS[${j}]}" "${TEST_FILE}" "${EVAL_DATA_TYPE}" "${ADD_LANG}"
                    done
