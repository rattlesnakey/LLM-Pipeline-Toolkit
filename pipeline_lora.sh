...
#! 这是原始的, 就不再用这个函数去 inference 了
#TODO: single node 的 inference 要修改一下
inference_lora(){
    TEMPERATURE=0.35
    TOP_P=0.85
    TOP_K=40
    ...
    #! output lora dir
    OUTPUT_FILE_DIR=${LORA_WEIGHTS_PATH}/predictions/${DATA_TYPE}/${SRC_LANG}-${TGT_LANG}/${PROMPT_TYPE}
    mkdir -p ${OUTPUT_FILE_DIR}

    OUTPUT_FILE_PATH=${OUTPUT_FILE_DIR}/${MODEL_TYPE}-for-${TEST_FILE_NAME}_prediction.jsonl
    LOG_PATH=${BASE_DIR}/projects/LLM_inference/logs/inference/lora/${DATA_TYPE}/${MODEL_NAME}/${PROMPT_TYPE}
    ...
    deepspeed --include=localhost:6 src/inference_v4.py \
...
ddp_inference_lora(){
    ...
    if [ ${USE_LORA} = 'True' ]; then
        OUTPUT_BASE_DIR=${LORA_WEIGHTS_PATH}
    else
        OUTPUT_BASE_DIR=${PRETRAINED_MODEL_PATH}
    ...
    #! set_decoding_type
    # if [ "${PROB_DECODE}" = "True" ]; then 
    #     DECODE_TYPE=probing
    # else
    #     DECODE_TYPE=generate
    # fi
    ...
    if [[ "${DATA_TYPE}" == *"X_"* ]]; then
        ...
        if [ "${4}" = "True" ]; then
            OUTPUT_FILE_DIR=${OUTPUT_BASE_DIR}/predictions/${DATA_TYPE}/${LANG}/${SRC_LANG}-${TGT_LANG}/${PROMPT_TYPE}/${EXAMPLE_TYPE}
        else
            OUTPUT_FILE_DIR=${OUTPUT_BASE_DIR}/predictions/${DATA_TYPE}/${LANG}/no-add-lang/${PROMPT_TYPE}/${EXAMPLE_TYPE}
        fi
        
    else 
        if [ "${4}" = "True" ]; then
            OUTPUT_FILE_DIR=${OUTPUT_BASE_DIR}/predictions/${DATA_TYPE}/${SRC_LANG}-${TGT_LANG}/${PROMPT_TYPE}/${EXAMPLE_TYPE}
        else
            OUTPUT_FILE_DIR=${OUTPUT_BASE_DIR}/predictions/${DATA_TYPE}/no-add-lang/${PROMPT_TYPE}/${EXAMPLE_TYPE}
        fi
    fi
...
eval_x_csqa(){
    declare -a EVAL_DATA_TYPES=('X_CSQA' 'X_CSQA_PIQA' 'X_CSQA_ARC_Easy' 'X_CSQA_ARC_Challenge') #'X_CSQA' 'X_CSQA_PIQA' 'X_CSQA_ARC_Easy' 'X_CSQA_ARC_Challenge'
...
eval_translation(){
    declare -a EVAL_DATA_TYPES=('translation_X-CSQA-ARC-Challenge')
    # declare -a TEST_LANGS=('en' 'zh' 'ar' 'bg' 'de' 'el' 'es' 'fr' 'hi' 'ru' 'sw' 'th' 'tr' 'ur' 'vi')
    # declare -a TEST_LANGS=('en' 'zh')
    EXTRA_LARGE=False
    # USE_EXP=True
    ...
    do 
        ...
        do
            ...
            do
                for ADD_LANG in False
                do 
                    for USE_EXP in False
                    do
                        
                        TEST_FILE=datas/X-CSR_datasets/X_CSQA_ARC_Challenge/en_translated_from_zh/demo+dev_v2_for_translation.jsonl
                        ...
                        # METRIC_DIR=${OUTPUT_MODEL_DIR}/metrics/${EVAL_DATA_TYPE}
                        # mkdir -p ${METRIC_DIR}
                        # python -u src/eval_translation.py \
                        #     --result_dir ${RESULT_DIR} \
                        #     --metric_dir ${METRIC_DIR} 
                    done
                done
...
TRAIN_DATASET_TYPE=alpaca_data+translation

# TRAIN_DATASET_TYPE=alpaca_data+instruction_mix
# DATA_FILE_PREFIX=alpaca_instruction_mix_
# DATA_FILE_SUFFIX=_zh_lang_data.json
# TRAIN_DATASIZES=('200' '1000' '5000' '10000')

# DATA_FILE_PREFIX=alpaca_data+en2zh-
# DATA_FILE_SUFFIX=.json
# TRAIN_DATASIZES=('100' '200' '300' '400' '500' '600' '700' '800' '900' '1000' '5000' '10000' '25000' '50000' '100000')
# TRAIN_DATASIZES=('10000')

DATA_FILE_PREFIX=alpaca_data+en2zh+zh2en-not-same-
# DATA_FILE_SUFFIX=.json
# TRAIN_DATASIZES=('500' '1000' '5000' '50000' '100000')
# TRAIN_DATASIZES=('100' '200' '300' '400' '500' '600' '700' '800' '900' '1000' '5000' '10000' '25000' '50000' '100000')
eval_x_csqa_different_size(){
    ...
    for ((i=0;i<${#MODEL_NAMES[@]};i++))
    do 
        for j in ${!TRAIN_DATASIZES[@]}
        do 
            train_lora "${PRETRAINED_MODELS[${i}]}" "${MODEL_NAMES[${i}]}" "${DATA_FILE_PREFIX}${TRAIN_DATASIZES[${j}]}${DATA_FILE_SUFFIX}" "${TRAIN_DATASET_TYPE}" "${MODE}" "${EXTRA_LARGE}" #! modify mode
            for EVAL_DATA_TYPE in ${EVAL_DATA_TYPES[@]}
            do
                for ADD_LANG in False
                do 
                    ...
                    do
                        ...
                        do
                            ...
                            ddp_inference_lora "${MODEL_NAMES[${i}]}+${DATA_FILE_PREFIX}${TRAIN_DATASIZES[${j}]}${DATA_FILE_SUFFIX}" "${TEST_FILE}" "${EVAL_DATA_TYPE}" "${ADD_LANG}" "${EXTRA_LARGE}" "${USE_EXP}" "${PROB_DECODE}" "${PROB_TEXT}" "${DECODING_MODE}"
                        done
            ...
            do
                ...
                do 
                    ...
                    do
                        for LANG in ${TEST_LANGS[@]}
                        do
                            TEST_FILE=datas/X-CSR_datasets/${EVAL_DATA_TYPE}/${LANG}/dev_v2.jsonl
...
eval_translation_different_size(){
    declare -a EVAL_DATA_TYPES=('translation')
  
    EXTRA_LARGE=False
    DECODING_MODE=sample #! decoding mode change to sample 
    # USE_EXP=True
    MODE=eval
    PROB_DECODE=False #! change to False, will use generate
    PROB_TEXT=False
    for ((i=0;i<${#MODEL_NAMES[@]};i++))
    do 
        for j in ${!TRAIN_DATASIZES[@]}
        do
            ...
            do
                for ADD_LANG in False
                do 
                    #! translation cannot use examplar
                    for USE_EXP in False
                    do
                        
                        TEST_FILE=datas/wmt23/test/en_zh_translation_test_set_1000.jsonl
