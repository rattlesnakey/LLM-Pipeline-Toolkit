...
train_adapter(){
    ...
    PROMPT_EPOCHS=8 #! 8 epochs

    SEED=42 #! default 42

    LR_LORA=1e-4
    LR_PROMPT=5e-3 #! 2e-5 for full training, 3e-4 or 2e-4 or 1e-4 for lora, prompt tuning 我记得要 1e-2 才能训的起来
    ...
    if [ ${LORA_TRAIN} = 'True' ] && [ ${PROMPT_TRAIN} = 'True' ]; then
        ...
        OUTPUT_MODEL_DIR=${OUTPUT_BASE_DIR}/prompt_with_lora_v2/two_stage/${MODEL_NAME}/spec_att_${SPEC_ATT}/${DATASET_TYPE}/${WANDB_RUN_NAME_PROMPT}-virtual-tokens_${PROMPT_TOKENS}
        #! 自动获取对应 Model name 和 data type 的 LORA WEIGHTS
        #! Lora 的 learning rate 和 prompt 不一样，所以 ..
        #! 加一下 prompt-tokens 的数量进去
    ...
    #! zero1 + fp16 for 7b lora
    #! zero1 + fp16 for 7b prompt tuning, but need to kill the holding script
...
ddp_inference_adapter(){
    ...
    # elif [ ${USE_LORA} = 'True' && ${USE_PROMPT} = 'False' ]; then
    #     ADAPTER_WEIGHTS_PATH=${OUTPUT_MODEL_DIR}
    #     #! prompt weight path will be NULL

    # elif [ ${USE_LORA} = 'False' && ${USE_PROMPT} = 'True' ]; then
    #     ADAPTER_WEIGHTS_PATH=${OUTPUT_MODEL_DIR}
    #     #! lora weights path set in train function, LORA_WEIGHTS_PATH is global var
    # fi
    ...
    deepspeed --include=localhost:${DEVICE} src/ddp_batch_inference_v4.py \
...
eval_x_csqa(){
    ...
    SPEC_ATTS=('True') #! only for prompt train
            ...
            do
                ...
                do
                    ...
                    do 
                        ...
                        do
                            #! 这边写错了
                            TEST_FILE=datas/X-CSR_datasets/${EVAL_DATA_TYPE}/${LANG}/test_pipeline.jsonl #! 有的是 dev.jsonl, 有的是 dev_v2.jsonl
...
eval_x_nli(){
    ...
    PROMPT_TOKENS=100
    MODE=eval #! eval or train
    SPEC_ATTS=('False') #! only for prompt train
