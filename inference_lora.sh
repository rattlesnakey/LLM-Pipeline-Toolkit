...
PRETRAINED_MODEL_PATH=${BASE_DIR}/pretrained_models/bloom/bloom-7b-mt
LORA_WEIGHTS_PATH=${BASE_DIR}/projects/LLM_inference/checkpoints/lora/official-bloom-7b-mt-test_pipeline_data.json-prompt-type_stanford-train-batch_16-lr_3e-4-epochs_3
...
#! output lora dir
OUTPUT_FILE_DIR=${LORA_WEIGHTS_PATH}/predictions/${DATA_TYPE}/${PROMPT_TYPE}
...
RUN_NAME=${MODEL_NAME}-${LORA_WEIGHTS_PATH}-${DATA_TYPE}-${PROMPT_TYPE}-temperature_${TEMPERATURE}-topk_${TOP_K}-topp_${TOP_P}-repetition_${REPETITION_PENALTY}-beams_${NUM_BEAMS}.log
...
deepspeed --include=localhost:2 src/inference_v3.py \
