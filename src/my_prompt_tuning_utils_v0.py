...
from peft import PeftModel, PeftModelForCausalLM, PromptTuningConfig, PromptLearningConfig, PeftType
import torch 
...
class MyPeftModel(PeftModel):
    ...
    def from_pretrained(cls, model, model_id, adapter_name="default", is_trainable=False, **kwargs):
        ...
        config = PromptTuningConfig.from_pretrained(model_id, subfolder=kwargs.get("subfolder", None))
        ...
        # model = MyPeftModelForCausalLM(model, config, adapter_name)
        model = PeftModelForCausalLM(model, config, adapter_name)
        # else:
...
class MyLLaMAModel(LLaMAModel):
    ...
    def forward(
        ...
        instruction_lens = None, #! for prompt tuning 
        padding_lengths = None, #! for prompt tuning 
        ...
        if use_special_attention_mechanism:
            ...
            #! each prompt token cannot attend other tokens 
        # attention_mask[:,:,:num_virtual_tokens,num_virtual_tokens:] = MASK_WEIGHT
            logger.warning_once(
                    "\nmodify the original attention mask for personal prompt tuning\n"
                    )
            # else:
            #     raise ValueError('Please passing the virtual tokens and instruction length')
        logger.warning_once(
                    "\nmodify done!\n"
                    )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            #! 在用 generation 函数的时候，model_kwargs 里面已经有 inputs_embeds，但是没有 input_ids 
            #! 在 training 阶段的时候是没有 inputs_embeds 的，所以要临时构造
            if inputs_embeds is None:
                #! 先得到 inputs_embeds
            ...
            #! 没有传 input_ids 进去，防止和 inputs_embed 冲突
        ...
        if isinstance(peft_config, PromptLearningConfig):
            ...
            if model_kwargs["past_key_values"] is None and peft_config.peft_type == PeftType.PREFIX_TUNING:
                ...
                if model_kwargs["past_key_values"] is None:
                    ...
                    #! instruction_lens is from kwargs, need to specify in model_args
                    #! other wise it cannot pass to model.forward function
                    model_kwargs["instruction_lens"] = kwargs['instruction_lens']
                    model_kwargs["padding_lengths"] = kwargs['padding_lengths']
                    # model_kwargs["use_special_attention_mechanism"] = False
