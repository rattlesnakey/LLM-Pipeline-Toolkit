...
#! add for loading extra large LM, e.g. 13b and saving zero3 lora adapter
...
def _tokenize_fn(strings: Sequence[str], tokenizer: transformers.PreTrainedTokenizer) -> Dict:
    ...
    #! each text truncation
    #! cause no batch, thus padding not work
    tokenized_list = [
        tokenizer(
            text,
            return_tensors="pt",
...
class IterDataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        #! padding here rather than tokenization period
...
def train():
    ...
    if training_args.lora_train:
        if 'llama' in model_args.model_name_or_path or 'opt' in model_args.model_name_or_path:
    ...
    WEIGHT_NAME = 'adapter_model.bin' if training_args.lora_train else 'pytorch_model.bin'
